...
export default function ApiTestPage() {
  ...
  // 💾 ฟังก์ชันกดเซฟปุ่มเดียว บันทึกคีย์ลงระบบจัดลำดับ Pool ของหลังบ้าน
  const saveAllKeys = () => {
    const updatedPool = [
      { 
        provider: 'thaillm', 
        url: 'https://thaillm.or.th/api/v1', // 🎯 แก้ไขเป็น URL จริงที่ถูกต้องเพื่อแก้ Error 404
        key: thaiLlmKey.trim(), 
    ...
    ].filter(item => item.key !== ''); // กรองเก็บเฉพาะช่องที่มีการกรอกคีย์จริง (มีคีย์เดียวก็ใช้งานได้)
    ...
    alert(`บันทึกข้อมูลเข้า API Pool เรียบร้อยครับพี่! ตรวจพบ ${updatedPool.length} ค่ายพร้อมทำงาน`);
  };
  ...
  const handleTestApi = async () => {
    ...
    try {
      ...
      const currentPool = [
        { provider: 'thaillm', url: 'https://thaillm.or.th/api/v1', key: thaiLlmKey.trim(), model: 'opentaigpt-thaillm-8b-instruct-v7.2' },
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '40px auto', padding: '20px', fontFamily: 'sans-serif' }}>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
        <h2 style={{ color: '#0f172a', margin: 0, fontSize: '20px' }}>ศูนย์ควบคุมระบบคีย์เดี่ยว (Multi-Provider API Pool Center)</h2>
      ...
      <div style={{ border: '1px solid #cbd5e1', borderRadius: '12px', padding: '24px', backgroundColor: 'white', boxShadow: '0 4px 6px -1px rgb(0 0 0 / 0.1)' }}>
        <h3 style={{ margin: '0 0 20px 0', color: '#1e293b', borderBottom: '2px solid #e2e8f0', paddingBottom: '10px', fontSize: '16px' }}>⚙️ แผงตั้งค่าและจัดการ Token Key</h3>
        
        {/* ช่องทางที่ 1: ThaiLLM Key */}
        <div style={{ marginBottom: '15px' }}>
          <label style={{ display: 'block', fontWeight: 'bold', fontSize: '14px', marginBottom: '6px', color: '#475569' }}>1. ThaiLLM API Key (ระบบหลักโครงการ)</label>
          <input 
            type="password" 
            value={thaiLlmKey}
        ...
        <div style={{ marginTop: '30px', paddingTop: '20px', borderTop: '2px dashed #e2e8f0' }}>
          <h4 style={{ margin: '0 0 12px 0', color: '#1e293b', fontSize: '15px' }}>🚀 ช่องทดสอบสัญญาณกระตุ้นการส่งคำสั่ง (Test Sandbox)</h4>
